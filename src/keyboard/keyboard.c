...
void keyboardHandler(struct IntRegisters *regs){
    char code = inPortB(0x60);
    char scanCode = code  & 0x7F; //this gives  what key is pressed ; lower 7 bits ; 7F = 0111 1111
    char press = code & 0x80; // this gives key is pressed or released ; higher one bit ; 80 = 1000 000

    // printf("Scan code: %d, Press: %d \r\n",scanCode,press);

    switch(scanCode && press == 0){
        case 1:
        case 29:
        case 56:
        case 60:
        ...
        break;
        case 58: //capsLock On
            if(!capsLockOn && press == 0){
                capsLockOn = true;
            }else if (capsLockOn && press == 0){
                capsLockOn = false;
            }
        break;
        default:
